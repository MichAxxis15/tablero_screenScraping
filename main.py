...
def execute_pipeline():
    try:
        console.rule("[bold cyan]INICIANDO PIPELINE DATUM SCRAPPING[/bold cyan]")

        console.rule("[bold blue]PASO 1: Inicializando Base de Datos[/bold blue]")
        with console.status("[yellow] Verificando base de datos...", spinner="dots"):
            initialize_database()
        logging.info("[green]✅ Base de Datos lista.[/green]\n" )

        console.rule("[bold blue]PASO 2: Extrayendo Artículos desde DATUM[/bold blue]")
        with console.status("[yellow]Ejecutando scrapper de articulos...", spinner="dots"):
            articulos.main()
        logging.info("[green]✅ Artículos actualizados correctamente.[/green]\n")
